...
def get_selected(state):
    ...
    for i, frage in enumerate(Fragen):
        ...
        state.answers[i].selected_choice = frage.get_selected_choice(symbol_no=state.selected_choices[f'frage_{i}'])
...
for i, frage in enumerate(Fragen):
    ans = Answer()
    ans.question = frage
    ans.selected_choice = frage.get_selected_choice(symbol_no="1")
    answers.append(ans)
